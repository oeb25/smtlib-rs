...
pub fn generate(mut f: impl std::io::Write) -> miette::Result<()> {
    let raw: RawSpec = toml::from_str(include_str!("./spec.toml")).into_diagnostic()?;
    let spec = raw.parse();

    writeln!(f, "// This file is autogenerated! DO NOT EDIT!\n").into_diagnostic()?;
    writeln!(f, "use crate::parse::{{Token, Parser, ParseError}};").into_diagnostic()?;
    writeln!(f, "use itertools::Itertools; use crate::lexicon::*;\n").into_diagnostic()?;

    for (name, s) in &spec.general {
        writeln!(f, "{}", s.rust_ty_decl_top(name)).into_diagnostic()?;
        writeln!(f, "{}", s.rust_display(name)).into_diagnostic()?;
        writeln!(f, "{}", s.rust_parse(name)).into_diagnostic()?;
        writeln!(f, "{}", s.rust_response(name)).into_diagnostic()?;
    }
