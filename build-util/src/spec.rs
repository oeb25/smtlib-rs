...
pub fn generate(mut f: impl std::io::Write) -> miette::Result<()> {
    ...
    writeln!(buf, "use crate::parse::{{Token, Parser, ParseError}};").into_diagnostic()?;
    writeln!(buf, "use itertools::Itertools; use crate::lexicon::*;\n").into_diagnostic()?;

    for (name, s) in &spec.general {
        writeln!(buf, "{}", s.rust_ty_decl_top(name)).into_diagnostic()?;
        writeln!(buf, "{}", s.rust_display(name)).into_diagnostic()?;
        writeln!(buf, "{}", s.rust_parse(name)).into_diagnostic()?;
        writeln!(buf, "{}", s.rust_response(name)).into_diagnostic()?;
    }

    let file = syn::parse_file(&buf).into_diagnostic()?;
    let pretty = prettyplease::unparse(&file);

    f.write_all(pretty.as_bytes()).into_diagnostic()?;
