...
pub fn generate(mut f: impl std::io::Write) -> anyhow::Result<()> {
    let raw: RawSpec = toml::from_str(include_str!("./spec.toml"))?;
    let spec = raw.parse();

    writeln!(f, "// This file is autogenerated! DO NOT EDIT!\n")?;
    writeln!(f, "use crate::parse::{{Token, Parser, ParseError}};")?;
    writeln!(f, "use itertools::Itertools; use crate::lexicon::*;\n")?;

    for (name, s) in &spec.general {
        writeln!(f, "{}", s.rust_ty_decl_top(name))?;
        writeln!(f, "{}", s.rust_display(name))?;
        writeln!(f, "{}", s.rust_parse(name))?;
        writeln!(f, "{}", s.rust_response(name))?;
    }
